...
# STUDENT_MODEL_PATH=/path/to/qwen3vl_2b_instruct
# STUDENT_MODEL_PATH=Qwen/Qwen3-VL-2B-Instruct
# STUDENT_MODEL_PATH=/path/to/qwen3vl_2b_instruct_sink_track_cross_attn
STUDENT_MODEL_PATH=/path/to/qwen3vl_8b_instruct_sink_track_cross_attn
# STUDENT_MODEL_PATH=Qwen/Qwen3-VL-8B-Instruct
# TEACHER_MODEL_PATH=Qwen/Qwen3-VL-4B-Instruct
TEACHER_MODEL_PATH=Qwen/Qwen3-VL-8B-Instruct
# TEACHER_MODEL_PATH=Qwen/Qwen3-VL-2B-Instruct
...
CUR_EXP_NAME="OPSD_8B_thinking_with_system_prompt_RE1_sink_track_cross_attn_100epochs"
PROJECT_NAME="verl_on_policy_distillation_example_geo3k_${CUR_EXP_NAME}"
ROLLOUT_DATA_DIR="/path/to/rollout_results/${CUR_EXP_NAME}"
...
EXP_NAME="fsdp/student-${STUDENT_MODEL}-teacher-${TEACHER_MODEL}-${CUR_EXP_NAME}/loss-${DISTILLATION_LOSS_MODE}-pg-${USE_POLICY_GRADIENT}"

ENFORCE_EAGER=True # true for faster debugging

############################ Paths ############################

# geo3k_train_path=/path/to/train_teacher_merged.parquet # with system prompt, teacher prompt includes summary info
# geo3k_train_path=/path/to/train_base.parquet # with system prompt, without teacher prompt
geo3k_train_path=/path/to/train.parquet # with system prompt, teacher prompt includes summary info
geo3k_test_path=/path/to/test.parquet

# visionR1_train_path=/path/to/train_vision.parquet
# visionR1_test_path=/path/to/test_vision.parquet

TRAIN_FILES="['$geo3k_train_path']"
TEST_FILES="['$geo3k_test_path']"

# TRAIN_FILES="['$visionR1_train_path']"
# TEST_FILES="['$visionR1_test_path']"
