...
class RayPPOTrainer:
    ...
    def _warn_rollout_logprob_once(self, warning_key: str, warning_msg: str) -> None:
        if not hasattr(self, "_rollout_logprob_warning_keys"):
            self._rollout_logprob_warning_keys = set()
        if warning_key in self._rollout_logprob_warning_keys:
            return
        self._rollout_logprob_warning_keys.add(warning_key)
        print(f"[rollout_data][log_prob] {warning_msg}")

    def _extract_token_logprobs_for_dump(
        self,
        token_logprobs: torch.Tensor,
        response_mask: Optional[torch.Tensor],
        field_name: str,
    ) -> Optional[list]:
        """Prepare token-level logprobs for JSON dump with response-aligned masking."""
        logprobs = token_logprobs.detach().cpu()
        if logprobs.dim() == 1:
            logprobs = logprobs.unsqueeze(0)
        if logprobs.dim() not in (2, 3):
            self._warn_rollout_logprob_once(
                warning_key=f"{field_name}_bad_ndim",
                warning_msg=f"skip {field_name}: expected 2D/3D logprobs but got shape {tuple(logprobs.shape)}.",
            )
            return None
        ...
        if mask.dim() != 2:
            self._warn_rollout_logprob_once(
                warning_key=f"{field_name}_bad_mask_ndim",
                warning_msg=f"skip {field_name}: expected 2D response_mask but got shape {tuple(mask.shape)}.",
            )
            return None
        if logprobs.shape[0] != mask.shape[0]:
            self._warn_rollout_logprob_once(
                warning_key=f"{field_name}_bad_batch",
                warning_msg=(
                    f"skip {field_name}: batch mismatch logprobs={tuple(logprobs.shape)} vs "
                    f"response_mask={tuple(mask.shape)}."
                ),
            )
            return None
        if logprobs.shape[1] != mask.shape[1]:
            if logprobs.shape[1] > mask.shape[1]:
                logprobs = logprobs[:, -mask.shape[1] :]
            else:
                self._warn_rollout_logprob_once(
                    warning_key=f"{field_name}_short_seq",
                    warning_msg=(
                        f"skip {field_name}: token dim shorter than response_mask, "
                        f"logprobs={tuple(logprobs.shape)} response_mask={tuple(mask.shape)}."
                    ),
                )
                return None

        return [lp[m].tolist() for lp, m in zip(logprobs, mask, strict=True)]
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            if "old_log_probs" in batch.batch:
                student_token_logprobs = self._extract_token_logprobs_for_dump(
                    token_logprobs=batch.batch["old_log_probs"],
                    response_mask=response_mask,
                    field_name="student_token_logprobs",
                )
                if student_token_logprobs is not None:
                    reward_extra_infos_to_dump["student_token_logprobs"] = student_token_logprobs

            if "teacher_logprobs" in batch.batch:
                teacher_token_logprobs = self._extract_token_logprobs_for_dump(
                    token_logprobs=batch.batch["teacher_logprobs"],
                    response_mask=response_mask,
                    field_name="teacher_token_logprobs",
                )
                if teacher_token_logprobs is not None:
                    reward_extra_infos_to_dump["teacher_token_logprobs"] = teacher_token_logprobs

            if "ref_log_prob" in batch.batch:
                ref_token_logprobs = self._extract_token_logprobs_for_dump(
                    token_logprobs=batch.batch["ref_log_prob"],
                    response_mask=response_mask,
                    field_name="ref_token_logprobs",
                )
                if ref_token_logprobs is not None:
                    reward_extra_infos_to_dump["ref_token_logprobs"] = ref_token_logprobs
    ...
    def init_workers(self):
        ...
        if self.use_teacher_policy:
            ...
            if self.distillation_config.teacher_prompt_key:
                self.config.data["teacher_prompt_key"] = self.distillation_config.teacher_prompt_key
        else:
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.use_teacher_policy and self.global_steps == 1:
                        has_teacher_prompt = batch.non_tensor_batch.get("has_teacher_prompt")
                        teacher_prompt_hit_rate = 0.0
                        if has_teacher_prompt is not None:
                            teacher_prompt_hit_rate = float(np.asarray(has_teacher_prompt, dtype=np.float32).mean())
                        metrics["distillation/teacher_prompt_hit_rate"] = teacher_prompt_hit_rate
                        metrics["distillation/teacher_prompt_enabled"] = float(
                            bool(self.distillation_config.teacher_prompt_key)
                        )
                        print(
                            f"[distillation] teacher_prompt_key={self.distillation_config.teacher_prompt_key}, "
                            f"teacher_prompt_hit_rate={teacher_prompt_hit_rate:.3f}"
                        )
