...
def distillation_loss(
    ...
    # ======================================================================== #
    # Custom: Cascade Truncation -> Confidence Weighting -> Hindsight Margin
    # ======================================================================== #

    margin_gamma = loss_config.margin_gamma
    use_confidence_weight = loss_config.use_confidence_weight
    cascade_epsilon = loss_config.cascade_epsilon

    # Compute teacher_max_prob once if needed by confidence or cascade
    teacher_max_prob = None
    if use_confidence_weight or cascade_epsilon > 0.0:
        teacher_lps = data.get("teacher_logprobs", None)
        if teacher_lps is not None:
            teacher_lps = no_padding_2_padding(teacher_lps, data)
            if teacher_lps.dim() == 3:
                teacher_max_lp = teacher_lps[..., 0]
            else:
                teacher_max_lp = teacher_lps.squeeze(-1)
            assert teacher_max_lp.shape == distillation_losses.shape, (
                f"teacher_max_lp shape {teacher_max_lp.shape} != distillation_losses shape {distillation_losses.shape}"
            )
            teacher_max_prob = torch.exp(teacher_max_lp)

    # 1. Cascading Truncation — remove noisy tail before other adjustments
    if cascade_epsilon > 0.0 and teacher_max_prob is not None:
        is_low = (teacher_max_prob <= cascade_epsilon).float()
        consec = torch.zeros_like(is_low[:, :1])
        counts = []
        for t in range(is_low.shape[1]):
            consec = (consec + is_low[:, t : t + 1]) * is_low[:, t : t + 1]
            counts.append(consec)
        consec_counts = torch.cat(counts, dim=1)
        ever_hit = (consec_counts >= loss_config.cascade_tolerance).float()
        cascade_off = torch.cummax(ever_hit, dim=1)[0]
        cascade_mask = 1.0 - cascade_off
        distillation_losses = distillation_losses * cascade_mask
        truncation_ratio = 1.0 - ((cascade_mask * response_mask).sum() / response_mask.sum().clamp_min(1))
        distillation_metrics["distillation/ood_truncation_ratio"] = truncation_ratio.item()

    # 2. Confidence Weighting — soft-scale surviving tokens by teacher certainty
    if use_confidence_weight and teacher_max_prob is not None:
        omega_t = teacher_max_prob.pow(loss_config.confidence_alpha)
        distillation_losses = distillation_losses * omega_t
        distillation_metrics["distillation/avg_confidence_weight"] = (
            (omega_t * response_mask).sum() / response_mask.sum().clamp_min(1)
        ).item()

    # 3. Hindsight Margin — protect correct trajectories after noise reduction
    if margin_gamma > 0.0 and "reward" in data:
        rewards = data["reward"]
        if rewards.dim() == 1:
            rewards = rewards.unsqueeze(-1)
        is_correct = (rewards > 0).float()
        margin = margin_gamma * is_correct
        abs_losses = distillation_losses.abs()
        shrunken = torch.relu(abs_losses - margin)
        distillation_losses = shrunken * distillation_losses.sign()
        protected_ratio = (
            ((distillation_losses == 0) & response_mask.bool()).float().sum() / response_mask.sum().clamp_min(1)
        )
        distillation_metrics["distillation/exploration_protected_ratio"] = protected_ratio.item()
