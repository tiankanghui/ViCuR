...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.teacher_prompt_key = config.get("teacher_prompt_key", "teacher_prompt")
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        video_data = doc.get(video_key, None) if video_key else None
                        image_data = doc.get(image_key, None) if image_key else None
                        ...
                        #     ]
                        #     print(f"images is not None")
                        # else:
                        #     images = None
                        #     print(f"images is None")
                        if image_data:
                            images = [
                                process_image(image, image_patch_size=self.image_patch_size) for image in image_data
                            ]
                        else:
                            images = None
    ...
    def _build_messages(self, example: dict, prompt_key: Optional[str] = None):
        ...
        raw = copy.deepcopy(example[prompt_key])
        # Parquet may contain single {"role","content"} or list[dict]
        ...
        # Keep original row fields intact so we can build both student and teacher prompts from one sample.
    ...
    def __getitem__(self, item):
        ...
        if self.teacher_prompt_key in row_dict and row_dict[self.teacher_prompt_key] is not None:
            row_dict["teacher_raw_prompt"] = self._build_messages(row_dict, prompt_key=self.teacher_prompt_key)
